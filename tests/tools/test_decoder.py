    ...
    def test_encode_url(self, client, mock_api):
        route = mock_api.post("/message").mock(
            return_value=httpx.Response(
                200, json={"jsonrpc": "2.0", "id": 1, "result": {"result": "%3Cscript%3E", "encoding": "url"}}
            )
        )
        result = client.decoder.encode("<script>", "url")
        assert result["result"] == "%3Cscript%3E"
        body = json.loads(route.calls.last.request.content.decode())
        assert body["params"]["arguments"]["data"] == "<script>"
        assert body["params"]["arguments"]["encoding"] == "url"

    def test_encode_base64(self, client, mock_api):
        mock_api.post("/message").mock(
            return_value=httpx.Response(
                200, json={"jsonrpc": "2.0", "id": 1, "result": {"result": "dGVzdA==", "encoding": "base64"}}
            )
        )
...
class TestDecoderDecode:
    def test_decode_with_encoding(self, client, mock_api):
        mock_api.post("/message").mock(
            return_value=httpx.Response(
                200,
                json={"jsonrpc": "2.0", "id": 1, "result": {"result": "<script>", "encoding_detected": "url", "success": True}},
            )
        )
    ...
    def test_decode_auto_detect(self, client, mock_api):
        route = mock_api.post("/message").mock(
            return_value=httpx.Response(
                200,
                json={"jsonrpc": "2.0", "id": 1, "result": {"result": "test", "encoding_detected": "base64", "success": True}},
            )
        )
        client.decoder.decode("dGVzdA==")
        body = json.loads(route.calls.last.request.content.decode())
        assert "encoding" not in body["params"] or body["params"]["arguments"]["encoding"] is None
...
class TestDecoderHash:
    def test_hash_sha256(self, client, mock_api):
        mock_api.post("/message").mock(
            return_value=httpx.Response(
                200,
                json={"jsonrpc": "2.0", "id": 1, "result": {"hash": "9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08", "algorithm": "sha256"}},
            )
        )
